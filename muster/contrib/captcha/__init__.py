...
class CaptchaWidget(Widget):
	...
	def render(self, name, value, attrs=None):
		...
		return mark_safe(u'''<input type="hidden" name="%(name)s" value="captcha.%(hiddentext)s"/><img src="%(imageurl)s" alt="" /><br/><input type="text" name="%(name)s" id="id_%(name)s" />''' % {'name':name,'hiddentext': self.hiddentext,'imageurl': self.imageurl })
